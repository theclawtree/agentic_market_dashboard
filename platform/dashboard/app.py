...
def load_news() -> pd.DataFrame:
    ...
    return pd.concat(dfs, ignore_index=True).drop_duplicates(subset=["url"]) if dfs else pd.DataFrame()
    ...
    if not analysis_df.empty:
        ...
        for _, row in analysis_df.iterrows():
            ...
            if not news_df.empty and "market_question" in news_df.columns:
                market_news = news_df[news_df["market_question"].str.contains(
                    question, case=False, na=False
                )]
            
            avg_sentiment = market_news["sentiment_score"].mean() if not market_news.empty and "sentiment_score" in market_news.columns else 0
            dominant = "neutral"
            ...
            if row.get("platform") == "kalshi":
                spread_display = f"{int(spread_cents_val)}¢" if pd.notna(spread_cents_val) else "N/A"
            else:
            ...
            opportunities.append({
                ...
                "volume": float(row.get("volume_24h", 0) if pd.notna(row.get("volume_24h")) else row.get("volume", 0) if pd.notna(row.get("volume")) else 0),
